...
def cross_validation_tuning(dataset, pruned_tree):
    folds = get_folds(dataset)
    accuracy_list = []
    recall_list = np.zeros((4, 10))
    precision_list = np.zeros((4, 10))
...
test_folds = get_folds(noisy_data)
(testtree, testdepth) = parameter_tuning(test_folds[0:9], 9)
print("depth",  testdepth)
print("find depth", find_depth(testtree))
print(evaluate(test_folds[9], testtree))
print(cross_validation(noisy_data))
pruned_tree = parameter_tuning_pruning(noisy_data, 4, testtree)
print(evaluate(test_folds[9], pruned_tree))
print("find depth", find_depth(pruned_tree))
print(cross_validation_tuning(noisy_data, pruned_tree))
